...
#define BASE_TABLE_SIZE 4000001
...
/*
 * This is the primitive hash function that I compared my function with
uint32_t getHash(uint32_t key, int32_t tableSize) {
    unsigned long int code = 0;
...
TABLE* resize(TABLE* oldTable) {
    ...
    //newTable->collisions = oldTable->collisions;
    //printf("\n%d\n", newTable->collisions);
...
int main(void) {
    ...
    table = insertTable(table, 100117181, "Smrdi");
    table = insertTable(table, 352500552, "Nesmrdi");
    table = insertTable(table, 515626156, "Haha");
